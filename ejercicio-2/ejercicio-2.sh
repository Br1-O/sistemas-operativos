...
declare -r -i OPCION_GENERAR_HTML=5
declare -r -i OPCION_SALVAR_TSV=6
declare -r -i OPCION_SALIR=7
    ...
    while [[ "$opcion" != "$OPCION_SALIR" ]]; do
        ...
        case $opcion in
            ...
            $OPCION_MOSTRAR)
                ...
                ;;
            $OPCION_GENERAR_HTML)
                generate_html inventario
                ;;
            $OPCION_SALVAR_TSV)
                save_data_to_tsv inventario
                ;;
            $OPCION_SALIR)
...
print_table_body() {
    ...
    for ((i=0; i<MAX; i++)); do
        ...
        if [[ ${i_ref[$i,activo]} -eq 1 ]]; then
            cat << EOF >> vista_inventario.html
        <tr>
            <td> ${i_ref[$i,id]} </td> <td> ${i_ref[$i,nombre]} </td> <td> $ ${i_ref[$i,precio]} </td>
        </tr>
EOF
        fi
    done
...
load_data_into_array_from_tsv(){
    ...
    while IFS=$'\t' read -r id nombre precio; do
        ...
        invent_ref["$id,activo"]=1

    done < datos_inventario.tsv
}

save_data_to_tsv(){
    declare -n invento_ref=$1
    declare -i i=0
    ...
    for ((i=0; i<MAX; i++)); do
        ...
        if [[ ${invento_ref[$i,activo]} -eq 1 ]]; then
            printf "%d\t%s\t%s\n" \
                "$i" \
                "${invento_ref[$i,nombre]}" \
                "${invento_ref[$i,precio]}" >> datos_inventario.tsv
        fi
...
alta() {
    declare -n prod_ref=$1
    declare -i id=${prod_ref[id]} 
    ...
    printf "Producto %d (%s) guardado correctamente con precio $%.2f.\n" \
...
modificar() {
    declare -n produc_ref=$1
    declare -i id=${produc_ref[id]} 
    ...
    if [[ ${inventario[$id,activo]} -eq 1 ]]; then
        ...
        printf "Producto %d (%s) modificado correctamente con precio $%.2f.\n" \
        "$id" "${produc_ref[nombre]}" "${produc_ref[precio]}"
